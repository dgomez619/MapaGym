 ...
 // REAL LOGIC: Check Local Storage for logged in user
  ...
  return (
    <div className="relative h-screen w-full bg-zinc-900 overflow-hidden font-sans text-gray-100">
      ...
      {/* MODALS */}
      {isAuthModalOpen && (
        <AuthModal onClose={() => setIsAuthModalOpen(false)} onLoginSuccess={(user) => setCurrentUser(user)} />
      )}
                ...
                <div className="p-4 relative z-20 -mt-6">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {getGymTags(gym).map(tag => (
                      <span key={tag} className="text-[10px] uppercase tracking-wider font-semibold bg-zinc-900/80 text-zinc-400 px-2 py-1 rounded border border-zinc-700">{tag}</span>
                    ))}
                  </div>
